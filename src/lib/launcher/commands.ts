...
const power = (id: PowerAction, icon: string, keywords: string[] = []) =>
  command(
    id,
    tr(`launcher.commands.${id}`),
    tr("launcher.subtitles.power"),
    icon,
    () => powerAction(id),
    keywords,
  )
...
export const systemCommands = (): Result[] => [
  ...
  command(
    "recycle",
    tr("launcher.commands.recycle"),
    tr("launcher.subtitles.system"),
    "lucide:trash-2",
    emptyRecycleBin,
    ["trash"],
  ),
  ...
  command(
    "eris-settings",
    tr("launcher.commands.erisSettings"),
    tr("launcher.subtitles.eris"),
    "lucide:settings",
    () => showWindow("settings"),
    ["preferences", "options"],
  ...
  command(
    "eris-panel",
    tr("launcher.commands.erisPanel"),
    tr("launcher.subtitles.eris"),
    "lucide:calendar",
    () => showWindow("panel"),
    ["todo", "agenda", "events"],
  ...
  command(
    "eris-edit",
    tr("launcher.commands.erisEdit"),
    tr("launcher.subtitles.eris"),
    "lucide:pencil-ruler",
    () => editMode(true),
    ["layout", "customize", "arrange"],
  ),
  {
    ...command(
      "eris-clipboard",
  ...
  command(
    "screenclip",
    tr("launcher.commands.screenclip"),
    tr("launcher.subtitles.windows"),
    "lucide:crop",
  ...
  windowsPage("quiethours", "quiethours", "lucide:moon-star", [
    "do not disturb",
    "dnd",
    "quiet hours",
  ]),
  windowsPage("printers", "printers", "lucide:printer", ["scanner", "print"]),
  windowsPage("defaultapps", "defaultapps", "lucide:app-window", [
    "default browser",
    "file associations",
  ]),
  ...
  windowsPage("powersleep", "powersleep", "lucide:battery-charging", [
    "battery",
    "sleep",
    "power plan",
  ]),
  ...
  windowsPage("dateandtime", "dateandtime", "lucide:clock", [
    "clock",
    "timezone",
  ]),
  windowsPage("regionlanguage", "regionlanguage", "lucide:languages", [
    "locale",
    "keyboard layout",
  ]),
  windowsPage("startupapps", "startupapps", "lucide:rocket", [
    "autostart",
    "boot",
    "login",
  ]),
  command(
    "windowsdefender",
    tr("launcher.commands.security"),
    tr("launcher.subtitles.windows"),
    "lucide:shield-check",
    () => openUrl("windowsdefender:"),
    ["defender", "antivirus", "virus", "firewall"],
  ),
  windowsApp("devmgmt", "devmgmt", "lucide:cpu", "devmgmt.msc", [
    "drivers",
    "hardware",
  ]),
  windowsApp("diskmgmt", "diskmgmt", "lucide:hard-drive", "diskmgmt.msc", [
    "partitions",
    "volumes",
    "format",
  ]),
  ...
  windowsApp("regedit", "regedit", "lucide:file-cog", "regedit", [
    "registry",
    "regedit",
  ]),
  windowsApp("eventvwr", "eventvwr", "lucide:scroll-text", "eventvwr.msc", [
    "logs",
    "events",
  ]),
  windowsApp("msinfo32", "msinfo32", "lucide:info", "msinfo32", [
    "specs",
    "about this pc",
    "msinfo",
  ]),
  windowsApp("charmap", "charmap", "lucide:type", "charmap", [
    "symbols",
    "unicode",
    "special characters",
  ]),
  ...
  windowsApp("mspaint", "mspaint", "lucide:paintbrush", "mspaint", [
    "draw",
    "image editor",
  ]),
